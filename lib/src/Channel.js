...
class Channel{
    ...
    constructor({
         ...
         * may contain three properties: lose, damage and deliver, as the timeout ids for those actions.
         * @private
         */
    ...
    send(packet){
        ...
        const timeouts = {};
        if(isLosingPacket){
            timeouts.lose = setTimeout(() => {
                this.losePacket(packet);
            }, this.delay / 2);
        }else{
            if(isDamagingPacket)
                timeouts.damage = setTimeout(() => {
                    this.damagePacket(packet);
                }, this.delay / 2);
            timeouts.deliver = setTimeout(() => {
