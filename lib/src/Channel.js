...
class Channel{
    ...
    constructor({
        ...
        this.delay = delay;
        /**
         * @member {Map} - A map with the current traveling packets as keys and as values, objects that
         * may contain three properties: lose, damage and deliver, as the Timeout objects for those actions.
         * @private
         */
    ...
    /**
     * Returns a number between 1/4 and 3/4 of the delay.
     * Used to randomly lose or damage a packet around the
     * middle of the trajectory, so suddenly losing or
     * damaging a packet just before delivery or after
     * emission doesn't happen in the visualization.
     * @private
     */
    _getRandomTime(){
     ...
     * traveling in this channel. If the packet is traveling and
     * is damaged, calls onPacketDamaged() and returns true, otherwise
     * returns false.
    ...
    /**
     * Override this method to intercept damaged packets.
     ...
     */
    pause(){
        this._travelingPackets.forEach(timeouts => {
            for(const timeout of Object.values(timeouts))
                timeout.pause();
        });
    }
